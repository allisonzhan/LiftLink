...
export default function EditGymSessionModal({
  sessionId,
  onClose,
  onSuccess,
}: {
  ...
  const fetchSession = async () => {
    try {
      ...
      if (response.ok) {
        const session: GymSession = await response.json();
        // Convert stored UTC date back to local time for datetime-local input
        ...
        setFormData({
          ...
          genderPreference: session.genderPreference || '',
          experiencePreference: session.experiencePreference || '',
          sameGenderOnly: session.sameGenderOnly || false,
      ...
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="sticky top-0 bg-white border-b border-orange-200 px-6 py-4 flex justify-between items-center z-10">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl font-bold text-gray-900">Edit Gym Session</h2>
          </div>
          <button
            onClick={onClose}
